...
static inline void swap_elements(void * * store, uint32_t index_one, uint32_t index_two) {
	void *element_one = store[index_one];
	void *element_two = store[index_two];
...
static void heap_up(heap_t *heap) {
	...
	void * * const offsetStore = heap->store - 1;
	...
	while (element_index > 1) {
		...
		if (heap->comparator(element, parent_element) >= 0) {
			return;
		}
...
static void heap_down(heap_t *heap) {
	...
	while (1) {
		...
		if (child_b_index <= heap->size) {
			...
			if (heap->comparator(element, child_a) <= 0 && heap->comparator(element, child_b) <= 0) {
				return;
			}
			
			if (heap->comparator(child_b, child_a) < 0) {
			...
			// One child
			if (heap->comparator(element, child_a) <= 0) {
					return;
			}
			swap_elements(offsetStore, element_index, child_a_index);
			element_index = child_a_index;
